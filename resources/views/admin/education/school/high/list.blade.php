...
@section('content')
    <div class="col-md-12">
        <form method="get" action="{{route('admin.school.high.filter')}}">
        ...
        <a class="btn btn-primary float-right" href="{{route('admin.school.high.export-excel')}}" style="margin-right: 5px">
            Tải xuống Excel
        </a>
    </div>
        ...
        <table id="school" class="table table-bordered table-striped" style="width: 7000px">
            ...
            @foreach($schools as $item)
                <tr>
                    ...
                    <td>{{$item['high']['total_of_grade_11']}}</td>
                    <td>{{$item['high']['total_of_grade_12']}}</td>
                    <td>{{$item['high']['total_of_student']}}</td>
                    <td>{{$item['high']['total_of_student_10']}}</td>
                    <td>{{$item['high']['total_of_student_11']}}</td>
                    <td>{{$item['high']['total_of_student_12']}}</td>
                    <td>{{$item['high']['total_of_all_employees']}}</td>
                    <td>{{$item['high']['total_of_manager']}}</td>
                    <td>{{$item['high']['total_of_teacher']}}</td>
                    <td>{{$item['high']['total_of_employees']}}</td>
                    <td>{{$item['high']['total_classroom']}}</td>
                    <td>{{$item['high']['total_function_room']}}</td>
                    <td>{{$item['high']['total_subject_room']}}</td>
                    <td>{{$item['high']['total_device_full']}}</td>
                    <td>{{$item['high']['total_device_not_full']}}</td>
                    <td class="text-center">
                        <a href="{{route('admin.school.high.form.edit', $item->id)}}"><i class="fa fa-edit"></i></a>
                        <a href="{{route('admin.school.high.delete', $item->id)}}"><i class="fa fa-trash"></i></a>
                    </td>
                </tr>
            @endforeach
            </tbody>
