...
@section('content')
    <div class="col-md-12">
        <form method="get" action="{{route('admin.personnel.teacher.filter')}}">
        ...
        <a class="btn btn-primary float-right" href="{{route('admin.personnel.teacher.form.get')}}">
            Tạo mới giáo viên
        </a>
        <a class="btn btn-primary float-right" href="{{route('admin.personnel.teacher.export-excel')}}" style="margin-right: 5px">
            Tải xuống Excel
        </a>
    </div>
        ...
        <table id="personnel" class="table table-bordered table-striped" style="margin-top: 0;width: 2000px">
            ...
            @foreach($teachers as $item)
                <tr>
                    ...
                    <td>{{$item->year}}</td>
                    <td>{{config('base.type_of_personnel')[$item->type_personnel]}}</td>
                    <td>{{config('base.level_of_personnel')[$item->level]}}</td>
                    <td class="text-center">
                        <a href="{{route('admin.personnel.teacher.form.edit', $item->id)}}"><i class="fa fa-edit"></i></a>
                        <a href="{{route('admin.personnel.teacher.delete', $item->id)}}"><i class="fa fa-trash"></i></a>
                    </td>
                </tr>
            @endforeach
            </tbody>
